...
export function CalendarHeader({ 
  ...
  return (
    <div className="flex flex-col gap-2 p-4 border-b border-border/60 bg-card rounded-t-xl">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center">
            ...
            <span className="text-sm font-medium text-muted-foreground border-l border-border/60 pl-2 ml-2 hidden sm:inline-block">
              {formatHijriDate(currentDate, language, 'monthYear')}
            </span>
          </div>
        </div>
      ...
      {/* Accuracy Disclosure for Hijri */}
      <div className="w-full text-[10px] text-muted-foreground flex items-center justify-end pr-2 mt-1 gap-1">
        <Info className="w-3 h-3" />
        {tr("calendar", "hijriReferenceNote") || "Gregorian dates are primary. Hijri dates are calculated for reference."}
      </div>
    </div>
  );
}
