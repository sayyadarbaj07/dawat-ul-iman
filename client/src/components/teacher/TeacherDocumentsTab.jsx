...
import { teacherDocumentsApi } from "@/lib/api";
import { 
...
export function TeacherDocumentsTab({ teacherId }) {
  ...
  const fetchDocuments = async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await teacherDocumentsApi.list(teacherId);
      setDocuments(res.data || []);
    } catch (err) {
      if (err.response?.status === 403) {
        setError(tr("documents", "unauthorized") || "You're not authorized to access teacher documents.");
      } else {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      await teacherDocumentsApi.upload(teacherId, formData);
      setUploadOpen(false);
      resetForm();
      fetchDocuments();
    } catch (err) {
  ...
  const handleReplace = async (e) => {
    ...
    try {
      await teacherDocumentsApi.replace(selectedDoc._id, formData);
  ...
  const handleVerify = async (doc) => {
    try {
      await teacherDocumentsApi.verify(doc._id);
      fetchDocuments();
    } catch (err) {
  ...
  const handleReject = async (e) => {
    ...
    setIsSubmitting(true);
    try {
      await teacherDocumentsApi.reject(selectedDoc._id, rejectionReason);
      setRejectOpen(false);
      resetForm();
      fetchDocuments();
    } catch (err) {
  ...
  const handleDelete = async () => {
    ...
    setIsSubmitting(true);
    try {
      await teacherDocumentsApi.delete(selectedDoc._id);
      setDeleteOpen(false);
      resetForm();
      fetchDocuments();
    } catch (err) {
    ...
    <div className="space-y-6" dir={isRtl ? "rtl" : "ltr"}>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">{tr("documents", "teacherDocumentsAndVerification")}</p>
        </div>
        {isAdmin && (
              ...
              <CardContent className="p-4 sm:p-6 flex flex-col md:flex-row gap-6 justify-between items-start md:items-center">
                ...
                {isAdmin && (
                  <div className="flex flex-wrap gap-2 w-full md:w-auto">
                    <Button variant="outline" size="sm" onClick={() => teacherDocumentsApi.view(doc._id)}>
                      <Eye className="w-4 h-4 mr-1" /> {tr("documents", "view")}
                    </Button>
                    <Button variant="outline" size="sm" onClick={() => teacherDocumentsApi.download(doc._id)}>
            ...
            <div className="space-y-2">
              ...
              <Select value={docType} onValueChange={setDocType} dir={isRtl ? "rtl" : "ltr"}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="teacher_photo">{tr("documents", "teacher_photo")}</SelectItem>
