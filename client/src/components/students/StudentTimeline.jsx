...
export function StudentTimeline({ studentId }) {
  ...
  const fetchTimeline = async (pageNumber) => {
    try {
      ...
      const res = await studentTimelineApi.getTimeline(studentId, { page: pageNumber, limit });
      if (res && res.data) {
        if (pageNumber === 1) {
          setEvents(res.data);
        } else {
  ...
  const getEventConfig = (eventType) => {
    switch (eventType) {
      ...
      case 'STUDENT_PROMOTED':
        return { icon: ArrowUpCircle, color: 'text-green-500', bg: 'bg-green-100 dark:bg-green-900/20' };
      case 'STUDENT_HOSTEL_ASSIGNED':
      case 'STUDENT_HOSTEL_UPDATED':
      case 'STUDENT_HOSTEL_TRANSFERRED':
      case 'STUDENT_HOSTEL_VACATED':
        return { icon: Home, color: 'text-purple-500', bg: 'bg-purple-100 dark:bg-purple-900/20' };
      case 'STUDENT_DOCUMENT_UPLOADED':
      case 'STUDENT_DOCUMENT_REPLACED':
        return { icon: FileText, color: 'text-indigo-500', bg: 'bg-indigo-100 dark:bg-indigo-900/20' };
      case 'STUDENT_DOCUMENT_VERIFIED':
        return { icon: CheckCircle, color: 'text-emerald-500', bg: 'bg-emerald-100 dark:bg-emerald-900/20' };
      case 'STUDENT_DOCUMENT_REJECTED':
        return { icon: XCircle, color: 'text-red-500', bg: 'bg-red-100 dark:bg-red-900/20' };
      case 'STUDENT_DOCUMENT_DELETED':
        return { icon: Trash, color: 'text-rose-500', bg: 'bg-rose-100 dark:bg-rose-900/20' };
      default:
